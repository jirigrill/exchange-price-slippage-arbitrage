...
def createSignature(clientId, publicKey, privateKey, nonce):
    ...
    privateKey = privateKey.replace('b ', '') if privateKey.startswith('b ') else privateKey
    ...
    signature = hmac.new(private_key_bytes, message.encode('utf-8'), digestmod=hashlib.sha256).hexdigest()
